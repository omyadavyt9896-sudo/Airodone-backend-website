...
class Phase77AccessControlTestCase(unittest.TestCase):
    ...
    def _seed_test_data(self):
        ...
        if not c2:
            cur.execute(
                """
                INSERT INTO courses (
                    title, slug, short_description, description, image, level, grade,
                    estimated_duration, learning_outcomes, course_benefits, certificate_description,
                    is_active, created_at, updated_at
                )
                VALUES (
                    ...
                    '/img/robot.jpg', 'Advanced', 2, '15 Hours of Learning',
    ...
    def test_01_unauthenticated_courses_accessible_publicly(self):
        ...
        # Should not display course detail cards on the top grade selection level
        self.assertNotIn(b"Short intro to drone tech", res.data)

        # Grade 1 view shows only Grade 1 course
        res_g1 = self.client.get("/courses?grade=1")
        self.assertEqual(res_g1.status_code, 200)
        self.assertIn(b"Drone Basics", res_g1.data)
        self.assertIn(b"Short intro to drone tech", res_g1.data)
        self.assertNotIn(b"Advanced Robotics", res_g1.data)

        # Grade 2 view shows only Grade 2 course
        res_g2 = self.client.get("/courses?grade=2")
        self.assertEqual(res_g2.status_code, 200)
        self.assertIn(b"Advanced Robotics", res_g2.data)
        self.assertNotIn(b"Drone Basics", res_g2.data)
    ...
    def test_12_unenrolled_student_sees_public_courses_in_catalogue(self):
        ...
        self.assertEqual(res_g1.status_code, 200)
        self.assertIn(b"Drone Basics", res_g1.data)
        self.assertIn(b"View Course Details", res_g1.data)
    ...
    def test_17_explicit_enrolled_and_access_functions(self):
        ...
        self.client.post(
            "/admin/users/new",
            data={"name": "Function Test", "email": "functest@example.com", "password": "Pass123456!", "student_class": "1", "is_active": "1"},
            follow_redirects=True,
        )
        conn = get_db_connection()
        cur = conn.cursor()
    ...
    def test_18_enrolled_student_access_granted(self):
        ...
        self.client.post(
            "/admin/users/new",
            data={"name": "Enrolled Student", "email": "enrolled@example.com", "password": "Pass123456!", "student_class": "1", "is_active": "1"},
            follow_redirects=True,
        )
        conn = get_db_connection()
        cur = conn.cursor()
        ...
        # Catalogue shows assigned course under grade
        res_cat = self.client.get("/courses?grade=1")
    ...
    def test_19_removing_course_access_preserves_student_progress_data(self):
        ...
        self.client.post(
            "/admin/users/new",
            data={"name": "Progress Test Student", "email": "progtest@example.com", "password": "Pass123456!", "student_class": "1", "is_active": "1"},
            follow_redirects=True,
        )
        conn = get_db_connection()
        cur = conn.cursor()
